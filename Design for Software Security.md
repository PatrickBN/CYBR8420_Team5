## 1. Data Flow Diagrams and Threat Modeling
 ...
 15. Data Flow 2fa confirmation Is Potentially Interrupted
  ...
  67. Spoofing of the Breach Reporter External Destination Entity 
   ...
   After reading through all 66 threats the team determine that the Cross-site forgery threat was unnecessary to mitigate as the BitWarden client uses API to pull data from data stores and not a browser. The threats 11, 30, 43, and 55 were removed leaving 61 threats. 

   ### 4.1 Gaps
   
   These vulnerabilities are where the majority of gaps lie in our mitigation strategy. Ensuring that data hasn't been tampered with or interupted, as with Threats 1, 8, 12, 14, or 19, can then lead to DoS. While it would keep the data safe it is still a gap that an attacker could take advatange of to disrupt our system. Another notable gap would be that the password db data store needs to function 
